    ...
    if (isset($_SESSION['loggedIN'])) {
        ...
        if(isset($_POST['btn_add'])) {
            ...
            if($goalpriority == "High" && $totalhigh == 3) {
                ...
                $input =$goalname;
               if(substr($input,0,3)=="APP"){
                $goalname='AAPL';
               }elseif (substr($input,0,3)=="CAR") {
                   $goalname='CARA';
               }elseif (substr($input,0,3)=="WAV") {
                   $goalname='WVE';
               }elseif (substr($input,0,3)=="ACC") {
                   $goalname='ARAY';
               }elseif (substr($input,0,3)=="EQU") {
                   $goalname='EQNR';
               }

                if (substr($input,-3)== 'Mid'){
                    $priority = substr($input, -3);
                }elseif (substr($input,-3)=='Low') {
                    $priority=substr($input, -3);
                }else{
                    $priority=substr($input, -4);
                }
                echo $goalname;
                echo "<br>";
                echo $priority;
                $query = "INSERT INTO goal(Name,Cost,Priority,Period,AccountID) VALUES ('$goalname','$goalcost', '$priority','$period','$accountID')";
